            ...
            if request.form['send_button_signup'] == 'send_otp_signup':
                ...
                if email in otp_signup:
                    if otp == otp_signup[email]:
                        ...
                        query = "CREATE TABLE {}_img (id INT AUTO_INCREMENT PRIMARY KEY, filename VARCHAR(255) NOT NULL, contents LONGBLOB NOT NULL)".format(id_uniq)
                        cur.execute(query)
                        print(request.form)
                        file_var = request.form['image_profile_signup']
...
def search_results():
    ...
    #  print(query)
     ...
     # execute a SELECT query on your database
     # sql = "SELECT * FROM SearchChecker WHERE name LIKE %s"
     # val = ('%' + query + '%',)
     # print(val)
     ...
     #     tk.append((results[i]['name'], results[i]['home']))
     #     print((results[i]['name'], results[i]['home']))
